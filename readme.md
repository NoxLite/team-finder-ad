...
## Описание
**TeamFinder** — это платформа, на которой разработчики, дизайнеры и другие специалисты смогут 
находить единомышленников для совместной работы над pet-проектами. 
У зарегистрированных пользователей есть возможность размещать здесь идеи проектов, 
находить команду и откликаться на опубликованные предложения.
...
* Дополнительные библиотеки: Pillow

---

# Руководство по развёртыванию проекта

Данное руководство поможет вам быстро поднять локальное окружение, настроить базу данных в Docker и запустить Django-приложение.

---

## Шаг 1. Настройка виртуального окружения

Для изоляции зависимостей проекта используйте стандартный модуль `venv`.

1. **Создайте виртуальное окружение** в корневой директории проекта:
   ```bash
   python3 -m venv venv
   ```

2. **Активируйте окружение в зависимости от вашей ОС:**
   * **Windows (PowerShell):** 
      ```bash 
...
Проект был разработан Соловьёвым Максимом, студентом 2 курса 
Воронежского государственного университета Факультета компьютерных наук

[**GitHub**](https://github.com/NoxLite)